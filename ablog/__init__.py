...
def create_app(config_name=None):
    ...
    app.jinja_env.filters['be_active'] = be_active
    app.jinja_env.filters['is_follow'] = is_follow
...
def register_logging(app):
    ...
    file_handler = RotatingFileHandler(os.path.join(basedir, 'logs/ablog.log'),
    ...
    mail_handler = SMTPHandler(
        ...
        toaddrs=['ADMIN_EMAIL'],
        subject='ablog Application Error',
...
def register_commands(app):
    ...
    def fakedata(category, post, comment):
        """Generate fake data."""
        from ablog.fakes import fake_user, fake_categories, fake_posts, fake_comments
...
def register_request_handlers(app):
    ...
    def query_profiler(response):
        for q in get_debug_queries():
            if q.duration >= app.config['ABLOG_SLOW_QUERY_THRESHOLD']:
                app.logger.warning(
